...
当前已完成 v0.3 PySide6 GUI 原型，下一步准备进入 v0.4 Smart Extraction。
...
当前已完成 v0.1 CLI 原型、v0.2 解压核心增强和 v0.3 PySide6 GUI 原型。
...
下一步应进入 v0.4 Smart Extraction。

建议任务顺序：

1. 真实桌面环境手动试用新增的内容预览、密码输入和打开目录按钮。
2. 设计 Smart Extraction 规则：单顶层目录不重复套壳，多顶层文件默认进同名目录。
3. 实现 Smart Extraction 的纯核心函数和单元测试。
4. 将 Smart Extraction 接入 CLI 和 GUI。
5. 评估 v0.3 是否需要截图或 GIF 作为 README 展示素材。

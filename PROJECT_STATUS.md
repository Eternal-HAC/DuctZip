...
当前处于 v0.1 CLI 原型完成、准备进入 v0.2 解压核心增强阶段。
...
1. 实现 `list_archive()`，解析 7-Zip 列表输出。
2. 实现 `test_archive()`，用于测试压缩包完整性。
3. 增加密码参数和密码错误测试。
4. 增加进度事件模型。
5. 增加路径穿越防护设计和测试。
3. 如果未来发布包内置 7-Zip，增加 `THIRD_PARTY_NOTICES.md` 并附带 7-Zip 许可证信息。
